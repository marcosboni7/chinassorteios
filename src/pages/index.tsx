import useSWR from 'swr';
import Raffle from '../backend/models/Raffle';
import { ReactElement, ReactNode } from 'react';
import ToasterLayout from '../components/ToasterLayout';
import SiteLayout from '../components/SiteLayout';
import Head from 'next/dist/shared/lib/head';

export default function rifas() {
  const { data: raffles } = useSWR<Raffle[]>('/api/raffles', async (url) => await (await fetch(url)).json(), {
    revalidateOnFocus: false
  });

  return (
    <>
      <Head>
        <title>Rifas do Wiliam</title>
      </Head>
      <div className="bg-[url('https://cdn.discordapp.com/attachments/1063180963439464641/1068624011786993814/shinabg.png')]"
        style={{
          height: 303
        }}>
      </div>
      <div className="container mx-auto mt-40  ">
        <h1 className='text-white font-bold '>
          <span className="border-b-4 border-lime-400">GRANDES</span> PREMIOS
        </h1>
        <div className="grid grid-cols-2 gap-3 mt-4">
          {raffles?.map((raffle) => {
            const date = new Date(raffle.createdAt!);

            const formattedDate = `${date.getDate()}/${date.getMonth() + 1}/${date.getFullYear()}`;

            return (
              <div className='flex rounded-md overflow-hidden bg-gray-600 w-full h-36'>
                <img className='w-6/1 h-full' src="data:image/jpeg;base64," alt="" />
                <div className="p-3 flex-grow">
                  <div className="relative flex flex-col h-full">
                    <h5 className='text-white'>
                      <a href={`/produto/${raffle.id}`}>{raffle.title}</a>
                    </h5>
                    <span className='text-white'>R$ {raffle.price}</span>
                    <small className='absolute bottom-0 text-white'>{formattedDate}</small>
                  </div>
                </div>
              </div>
            );
          })}
        </div>
      </div>
      <div className='mt-[100px] '>
        <div className="container text-3xl mx-auto mt-40  ">
          <h1 className='text-white font-bold '>
            <span className="border-b-4 border-lime-400">OS</span> GANHADORES
          </h1>
        </div>
      </div>
    </>
  )
}

rifas.getLayout = (page: ReactElement) => {
  return (
    <SiteLayout>
      {page}
    </SiteLayout>
  );
};